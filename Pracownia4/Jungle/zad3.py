...
class Jungle:
    ...
    def undo_move(self, m, target_val, old_p_count):
        self.curplayer = 1 - self.curplayer

        if m is None:
            return

        self.peace_counter = old_p_count

        pos1, pos2 = m
        x, y = pos1
        x2, y2 = pos2

        self.board[y][x] = self.board[y2][x2]
        self.board[y2][x2] = target_val

        if target_val:
            pl2, pc2 = target_val
            self.pieces[pl2][pc2] = pos2
...
class Player(object):
    ...
    def find_move(self):
        ...
        if moves:
            ...
            _, target = move
            p_count_before = self.game.peace_counter
            self.game.do_move(move)
            # self.game.undo_move(move, target, p_count_before)
            # self.game.do_move(move)
